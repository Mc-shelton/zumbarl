...
import { BUSINESS_VIEWER, useViewerProfile } from "../../auth/viewerProfile";
...
export function BusinessWorkspaceHeader({
  ...
  const viewer = useViewerProfile(BUSINESS_VIEWER);
  const headerTitle = title || `Welcome back, ${viewer.name}!`;
  const handlePrimaryAction = onPrimaryAction || onCreateOpportunity;
  const canUsePrimaryAction =
    (primaryActionHref || handlePrimaryAction) &&
    hasAccess(primaryActionAccess);
  ...
  return (
    <header className="business-workspace-header">
      ...
      <CampusTopActions
        ...
        menuItems={[
          { label: "Business profile", href: "/business/company-profile" },
